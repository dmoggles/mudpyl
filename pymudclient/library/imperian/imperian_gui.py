...
class AffLabel(BlackEventBox):
    def __init__(self,name,short_name,light):
        ...
        self.light=light
        if light:
            self.label.modify_fg(gtk.STATE_NORMAL, gtk.gdk.Color(PINK))
        else:
            self.label.modify_fg(gtk.STATE_NORMAL, gtk.gdk.Color(PURPLE))
    ...
    def set(self, status):
        ...
        if self.status == False:
            ...
            if self.light:
                self.modify_bg(gtk.STATE_NORMAL, gtk.gdk.Color(DARK_RED))
            else:
...
class AffGroup(BlackEventBox):
    def __init__(self, cure_name, affs, panel=None):
        ...
        self.modify_bg(gtk.STATE_NORMAL,gtk.gdk.Color(BG_DARK_GRAY))
    ...
    def set_value(self, seconds):
        if seconds <= self.max_seconds:
            ...
            if value == 0:
                ...
                pass
                #self.pb.modify_fg(gtk.STATE_PRELIGHT, gtk.gdk.Color(GREEN))
            elif value < 0.5:
...
class ToggleLabel(BlackEventBox):
    ...
    def set_state(self, state):
        if state:
            self.label.modify_fg(gtk.STATE_NORMAL, gtk.gdk.Color(BG_BLACK))
...
class ShieldBox(BlackEventBox):
    def __init__(self):
        BlackEventBox.__init__(self)
        self.shield=ToggleLabel('Shield',RED)
        self.rebound=ToggleLabel('Rebounding',RED)
        self.prism=ToggleLabel('Prismatic',RED)
        self.curseward=ToggleLabel('Curseward',PURPLE)
...
class CharDataLabel(BlackEventBox):
    def __init__(self, name):
        ...
        self.data_label.modify_fg(gtk.STATE_NORMAL, gtk.gdk.Color(OFF_WHITE))
...
class LimbDamageLabel(BlackEventBox):
    ...
    def update(self, damage, hits_left, confirmed_damage, hits):
        ...
        c={0:OFF_WHITE, 0.33:YELLOW, 0.66:ORANGE}
...
class EnemyPanel(BlackEventBox):
    def __init__(self):
        ...
        self.name_label.modify_fg(gtk.STATE_NORMAL, gtk.gdk.Color(RED))
